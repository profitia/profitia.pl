...
const PORT = 3302
const EVENT_LOG_PATH = path.join(os.tmpdir(), `profitia-recruitment-etap4-browser-${Date.now()}.json`)
...
async function main() {
  ...
  process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY = 'recruitment-etap4-browser-site-key'
  process.env.RECRUITMENT_NOTIFICATION_TO = 'monika.osiecka@profitia.pl'
  process.env.RECRUITMENT_CV_STORAGE_PATH = mkdtempSync(path.join(os.tmpdir(), 'profitia-recruitment-etap4-browser-storage-'))
  ...
  const sendEmail = async (input: HomeplSmtpEmailInput): Promise<HomeplSmtpSendResult> => {
    ...
    return {
      ...
      rejected: [],
      pending: [],
      messageId: `<recruitment-etap4-browser-${branch}@test>`,
  ...
  server.listen(PORT, HOSTNAME, () => {
    console.log(`RECRUITMENT_ETAP4_TEST_SERVER_READY http://${HOSTNAME}:${PORT}`)
    console.log(`RECRUITMENT_ETAP4_EVENT_LOG_PATH=${EVENT_LOG_PATH}`)
    console.log(`RECRUITMENT_ETAP4_STORAGE_ROOT=${process.env.RECRUITMENT_CV_STORAGE_PATH}`)
  })
