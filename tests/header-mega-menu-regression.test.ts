...
test('desktop mega menu uses one canonical child card format and lazy-mounted hero previews', () => {
  ...
  assert.match(header, /const HEADER_SURFACE_CLASS = 'bg-\[rgba\(255,255,255,0\.96\)\] backdrop-blur-md border-gray-100\/80'/)
  assert.match(header, /const showStableHeaderSurface = showScrolled \|\| Boolean\(activeDesktopMenu\)/)
  assert.match(header, /showStableHeaderSurface[\s\S]*?`\$\{HEADER_SURFACE_CLASS\} border-b shadow-/)
  assert.match(header, /aria-expanded=\{menuOpen\}/)
  assert.match(header, /aria-controls=\{`desktop-menu-\$\{menu\.id\}`\}/)
  assert.match(header, /activeDesktopMenu &&/)
  assert.match(header, /activeDesktopMenu\.desktopColumns === 3 \? 'w-\[760px\]' : 'w-\[510px\]'/)
  assert.match(header, /max-w-\[calc\(100vw-2rem\)\]/)
  assert.match(header, /rounded-2xl border \$\{HEADER_SURFACE_CLASS\} shadow-/)
  assert.doesNotMatch(header, /rounded-2xl border border-gray-100 \$\{HEADER_SURFACE_CLASS\}/)
  assert.match(header, /grid-cols-\[repeat\(3,244px\)\]/)
  assert.match(header, /grid-cols-\[repeat\(2,244px\)\]/)
  assert.match(header, /grid justify-center gap-1\.5 p-2/)
