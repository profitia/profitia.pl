...
async function testGracefulDegradation(): Promise<void> {
  ...
  await test("Returns PL fallback response", "degradation", () => {
    ...
    assert(response.includes("/doradztwo/"), "Fallback should include localized service links");
  });
  ...
  await test("Static recommendations for PL", "degradation", () => {
    ...
    assert(recs[0].href.startsWith("/doradztwo/"), "Should have valid localized hrefs");
  });
